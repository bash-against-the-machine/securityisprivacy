---
title: "Securing Your Small/Home Office (SOHO) Network: Introduction"
...
- Dual RJ45 ports (may also say 2 RJ45 or 2 Ethernet ports)
...
purpose as they tend to excell at their singular purpose versus multifunction devices that may be average across multiple
functions.
...
Small and home office networks are typically "flat" (all devices share the same network). This means a single compromised
device (a guest on WiFi, a vulnerable IoT gadget, a hacked printer) can expose every other device to attack and enable
rapid ransomware spread.

The solution is **network segmentation using VLANs**. By placing a pfSense firewall between the ISP modem and a managed
switch, traffic between network segments (trusted LAN, IoT, guest) can be strictly controlled so that devices on one VLAN
cannot reach devices on another. A dedicated Admin Device manages the firewall, and access to the firewall's web interface
from non-admin VLANs is blocked.

The required equipment beyond your ISP modem is modest: a mini PC with dual Ethernet ports (to run pfSense as the
firewall, ~$300), a managed switch that supports VLANs such as the TP-Link TL-SG105E (~$25–$45), and two wireless access
points for separate SSIDs (~$25-$45 each). A main laptop or PC with at least 16 GB of RAM acts as the Admin Device. The
ISP modem's built-in WiFi should be disabled once the segmented network is in place.

#### Next (Securing Your Small/Home Office (SOHO) Network: Installing pfSense)
